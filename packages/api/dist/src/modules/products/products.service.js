...
let ProductsService = class ProductsService {
    ...
    async create(dto, userId) {
        ...
        if (dto.barcode) {
            ...
            if (barcodeExists)
                throw new common_1.ConflictException(`Barcode "${dto.barcode}" is already in use`);
        }
    ...
    async findAll(params) {
        const { query, categoryId, schoolId, gender, isActive, lowStock, page = 1, limit = 20, sortBy = 'name', sortOrder = 'asc', } = params;
        const where = {};
        ...
        if (lowStock) {
            where.variants = {
                some: {
                    currentStock: { lte: this.prisma.productVariant.fields.reorderLevel },
                },
            };
        }
    ...
    async findByBarcode(barcode) {
        ...
        throw new common_1.NotFoundException(`No product found for barcode: ${barcode}`);
    }
    async findBySku(sku) {
        const product = await this.prisma.product.findUnique({
            where: { sku },
        ...
        if (!product) {
            ...
            if (!variant)
                throw new common_1.NotFoundException(`No product/variant found for SKU: ${sku}`);
            return variant;
        }
        return product;
    }
    ...
    async addVariant(productId, dto) {
        ...
        if (existing)
            throw new common_1.ConflictException(`Variant SKU "${dto.sku}" already exists`);
